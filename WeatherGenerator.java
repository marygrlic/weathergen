// @author: Mary Grlic
// meg323@scarletmail.rutgers.edu
// 214001099
...
public class WeatherGenerator {
    ...
    public static int[] forecastGenerator(double drywetProbability, double wetwetProbability, int numberOfDays) {
        ...
        if (StdRandom.uniform() > 0.5) { // first day wet or dry at random
            forecast[0] = 2; // dry if ranges [0, 0.5)
        }
        else {
            forecast[0] = 1; // wet if ranges (0.5, 1]
        }

        for (int i = 1; i < numberOfDays; i++) { // traverse through array
            if (forecast[i-1] == 2) {
    ...
    public static int[] oneMonthForecast(int numberOfLocations, int month, double longitude, double latitude) {
        double[][] drywetArr = new double[4100][14];
        double[][] wetwetArr = new double[4100][14];
        double[] drywetProb = new double[12];
        double[] wetwetProb = new double[12];
        int[] oneMonthForecast = new int[numberOfDaysInMonth[month]];

        populateArrays(drywetArr, wetwetArr);
        populateLocationProbabilities(drywetProb, wetwetProb, longitude, latitude, drywetArr, wetwetArr);

        double probabilitydw = drywetProb[month];
        double probabilityww = wetwetProb[month];
        oneMonthForecast = forecastGenerator(probabilitydw, probabilityww, numberOfDaysInMonth[month]);
        return oneMonthForecast;
    }
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        int spellLength = 0;
        int longestSpellLength = 0;
        for (int i = 0; i < forecast.length; i++)
        {
            if (forecast[i] == mode)
            {
                spellLength ++;
                if (spellLength > longestSpellLength) {
                    longestSpellLength = spellLength;
                }
            }
            else {
                spellLength = 0;
    ...
    public static void main (String[] args) {
        ...
        StdRandom.setSeed(1680213284665L); 
